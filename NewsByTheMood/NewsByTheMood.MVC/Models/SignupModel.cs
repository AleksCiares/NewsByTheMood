...
namespace NewsByTheMood.MVC.Models
{
    ...
    public class SignupModel
    {
        ...
        [Remote(action:"CheckUserName", controller: "Auth", HttpMethod = "Post", ErrorMessage = "Username is not available")]
        public required string Username { get; set; }

        [Required]
        // Need turn on regex ignore case
        [RegularExpression(@"^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$", 
            ErrorMessage = "Email is invalid or already taken")]
        [Remote(action: "CheckEmail", controller: "Auth", HttpMethod = "Post", ErrorMessage = "Email is invalid or already taken")]
